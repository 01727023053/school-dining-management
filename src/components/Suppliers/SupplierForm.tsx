...
const SupplierForm: React.FC<SupplierFormProps> = ({ onClose, onSave }) => {
  ...
  const totalDue = formData.previousDue + formData.todaysBill - formData.todaysDeposit;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addSupplier({
      ...
      todaysBill: formData.todaysBill,
      todaysDeposit: formData.todaysDeposit,
      totalDue: totalDue,
      lastUpdated: new Date().toISOString()
    });
    
    toast.success('সরবরাহকারী সফলভাবে যোগ করা হয়েছে');
    onSave();
  };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'previousDue' || name === 'todaysBill' || name === 'todaysDeposit' 
        ? Number(value) : value
    }));
  };
          ...
          <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 font-bengali">আজকের বিল (৳)</label>
                  <input
                    type="number"
                    name="todaysBill"
                    value={formData.todaysBill}
                    onChange={handleChange}
                    min="0"
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="০"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 font-bengali">আজকের জমা (৳)</label>
                  <input
                    type="number"
                    name="todaysDeposit"
                    value={formData.todaysDeposit}
                    onChange={handleChange}
                    min="0"
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    placeholder="০"
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <div className="mt-2 text-sm text-blue-700 font-bengali">
                  পূর্বের বকেয়া (৳{formData.previousDue}) + আজকের বিল (৳{formData.todaysBill}) - আজকের জমা (৳{formData.todaysDeposit}) = ৳{totalDue}
                </div>
              </div>
